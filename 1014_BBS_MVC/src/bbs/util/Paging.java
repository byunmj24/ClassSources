...
public class Paging {
	...
	public Paging(int nowPage, int rowTotal, int blockList, int blockPage) {
		...
		//이전 기능과 다음 기능을 초기화 한다.
		//isPrePage = isNextPage = false;
		isPrePage = false;
		isNextPage = false;
		
		//입력된 전체 게시물의 수를 통해 전체 페이지 수(totalPage)를 구한다.
		/*
		...
		*/
		...
		//현재 페이지의 값이 전체 페이지의 값보다 크다면 전체 페이지 값을 현재 페이지 값으로 지정한다.
		...
		//끝페이지의 값이 전체 페이지의 값보다 크면 끝페이지의 값을 전체 페이지 값으로 지정하자. 
		if(endPage > totalPage)
			endPage = totalPage;
		
		//현재 페이지 값에 의해 시작 게시물의 행번호와 끝 게시물의 행번호를 지정하여
		//현재 페이지에 보여질 게시물 목록을 얻을 준비를 하자.
		begin = (nowPage - 1) * blockList + 1;
		end = begin + blockList - 1;
		
		//이전기능 가능 여부 확인(startPage가 1이 아닌 경우)
		if(startPage > 1)
			isPrePage = true;
